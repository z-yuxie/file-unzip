 ...
 * Snappy格式解压策略实现
 * <p>
 * 该类继承自 {@link AbstractCompressedUnzipStrategy}，专门用于处理Snappy格式的压缩文件解压。
 * 使用Apache Commons Compress库进行解压，支持标准的Snappy格式。
 * </p>
 * <p>
 * 主要特点：
 * <ul>
 *   <li>支持标准Snappy格式：兼容Snappy压缩规范</li>
 *   <li>支持自动检测：可以自动识别Snappy格式</li>
 ...
 * </p>
 * <p>
 * 注意：Snappy格式通常用于压缩单个文件，如果解压后的数据是TAR格式，
 * 会自动切换到TAR解压策略进行处理。
 * </p>
 *
 * @author yuxie
 ...
 * @see UnzipConfig
 * @see org.apache.commons.compress.compressors.snappy
 */
@Slf4j
public class SnappyUnzipStrategy extends AbstractCompressedUnzipStrategy {
    ...
    public SnappyUnzipStrategy(UnzipConfig unzipConfig) {
        super(unzipConfig, CompressionFormat.SNAPPY);
    }
    
    /**
     * 创建压缩输入流
     * <p>
     * 使用Apache Commons Compress库创建Snappy解压缩输入流。
     * </p>
     *
